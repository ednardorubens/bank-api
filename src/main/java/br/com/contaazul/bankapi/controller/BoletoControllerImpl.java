...
public class BoletoControllerImpl implements BoletoController {
    ...
    @ApiResponses({
        @ApiResponse(code = 200, message = "Ok", response = BoletoEntity.class, responseContainer = "List")
    })
    @Override
    ...
    @ApiResponses({
        @ApiResponse(code = 200, message = "Ok", response = BoletoEntity.class),
        @ApiResponse(code = 404, message = "Bankslip not found with the specified id")
    })
    @Override
    @GetMapping(path = "/{id}", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public BoletoEntity buscar(@ApiParam(value = "Id do boleto", required = true) @PathVariable("id") final UUID boletoId) {
        return boletoService.buscar(boletoId);
    }

    @ApiOperation(value = "Create a bankslip")
    @ApiResponses({
        @ApiResponse(code = 201, message = "Bankslip created", response = BoletoEntity.class),
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @DeleteMapping("/{id}")
    public void cancelar(@ApiParam(value = "id do boleto", required = true) @PathVariable("id") final UUID boletoId) {
